...
#include <string>
using namespace std;
/* Реализация ЗАКРЫТОЙ хэш таблицы на с++
    Структуры названы по образцу на Википедии
   Будут пары (ключ, значение) пока типа char[]
...
// вычисление хэша
HASH_V eval_hash(KEY k) // ПРИМИТИВНАЯ - просто складываем коды символов
{                       // результат мы используем как индекс в массиве корзин   
    HASH_V result = 0;
    KEY kk = k; // возьмем копия переданного адреса и будем менять ее
    while (*kk) { // пока не встретим конец строки - а это в Си/Си++ код ноль
          result+=(*kk);
          kk++; // увеличиваем адрес (обойдемся пока без kk++ ;-)
    }      
    return result;
}
...
int ht_delete( HashTable *table, KEY k_del) // удаляет пару с заданным ключем
{
	...
	do {  // обойдем весь список с таким хэшем
        ...
        return 2; // хэш совпал, а ключ нет !!!
}
...
int main()
{
    ...
    // наполним таблицу
    std::cout << "============= TEST ADD i*10 ===============\n";
    for (char j=0; j<10;j++) {  // 10 элементов добавим
        ...
        sprintf(entr->k,"%d",j*10); // так преобразуем число j*10 в строку и копируем по адресу entr->k
        sprintf(entr->v,"value %d",j*10+5); // значение формируем и копируем по адресу entr->v 
...
// удалим элемент 60
    ht_delete(table, "60");
    // выведем измененный массив на экран - не должно быть (50, value 50)
